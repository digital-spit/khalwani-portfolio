...
> **Correction (2026-07-03, during execution):** 16.2.10 does NOT clear the bundled postcss moderate — GHSA-qx2v-qp2m-jg93 postcss is vendored inside `next` itself and the advisory range covers every next release up to 16.3.0-canary.5. After the bump + non-force `npm audit fix`: **0 high / 0 critical / 0 low, 2 moderate** (both the vendored postcss chain; `npm audit fix --force` would "fix" it by downgrading to next@9.3.3 — rejected). Acceptance (0 high/critical) met; the residual moderate rides along until Next ships a release with postcss ≥8.5.10.
